...
answer, not comment) length for each post.
...
and if it's a question write to standard output the id as primary key
and the length of the post as the value. If it's an answer output the
abs_parent_id as primary key and the length of the post as the value.
In addition we will want to add a marker to each output. 'A' for
questions and 'B' for answers.

We are only interested in questions and answers. So, if the node_type
doesn't match one of these we will skip the record and move to the
next.
...
for record in reader:
    ...
    if len(record) == 19:
        ...
        if node_type == 'question':
            ...
            marker = 'B'
            print "{0}\t{1}\t{2}".format(abs_parent_id, marker,
            str(len(body)))
